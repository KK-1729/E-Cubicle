...
app.get("/todo", function(req, res) { //the home route
...
app.post("/todo", function(req, res) { //whenever a new list item is added
  const day = date.getDate();
  ...
  if (pageTitle === day) {
    ...
    List.findOne({
      ...
      if (!err) {
        ...
        found.save();
        res.redirect("/todo" + pageTitle);
      }
    })
  }
});
app.post("/todo/delete", function(req, res) { //this is called whenever the checkbox is ticked
...
app.get("/todo/:newList", function(req, res) { //this a custom url list
...
app.get("/todo/work", function(req, res) { //this is the worklist
  res.render("list", {
    listTitle: "Work List",
